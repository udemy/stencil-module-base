  ...
  build-and-test:
    ...
    steps:
      ...
      - name: Set git Credentials
        run: |
          git config --global --add url."https://{{ "${{ steps.generate_token.outputs.token }}" }}:x-oauth-basic@github".insteadOf ssh://git@github
          git config --global --add url."https://{{ "${{ steps.generate_token.outputs.token }}" }}:x-oauth-basic@github".insteadOf https://github
          git config --global --add url."https://{{ "${{ steps.generate_token.outputs.token }}" }}:x-oauth-basic@github".insteadOf git@github
      - name: Set git Credentials
        run: |
