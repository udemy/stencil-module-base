  ...
  build-and-test:
    ...
    steps:
      ...
      - name: Set git Credentials
        run: |
          git config --global --add url."https://x-access-token:{{ "${{ steps.generate_token.outputs.token }}" }}@github".insteadOf ssh://git@github
          git config --global --add url."https://x-access-token:{{ "${{ steps.generate_token.outputs.token }}" }}@github".insteadOf https://github
          git config --global --add url."https://x-access-token:{{ "${{ steps.generate_token.outputs.token }}" }}@github".insteadOf git@github
      - name: Set git Credentials
        run: |
          git clone https://github.com/udemy/eng-team-management.git
      - name: Checkout
